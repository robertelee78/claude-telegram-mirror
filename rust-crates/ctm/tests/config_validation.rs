...
/// ADR-015: the default approval wait + buffer must equal the PreToolUse hook timeout
/// the installer registers (310), so the approval hook is never cancelled before its
/// wait completes. This locks the "behavior-preserving at defaults" property and guards
/// against the two numbers drifting apart.
#[test]
fn approval_wait_default_and_buffer_match_registered_timeout() {
    assert_eq!(config::DEFAULT_APPROVAL_WAIT_SECS, 300);
    assert_eq!(config::APPROVAL_HOOK_TIMEOUT_BUFFER_SECS, 10);
    assert_eq!(
        config::DEFAULT_APPROVAL_WAIT_SECS + config::APPROVAL_HOOK_TIMEOUT_BUFFER_SECS,
        310,
