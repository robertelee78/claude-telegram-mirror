...
- **Codex** — two halves, both automatic. Outbound: ctm adds its own entries to
...
- **Codex approvals are answered at the terminal, not from Telegram.** Codex's approval
  hook carries no request id, and answering from a blocking hook would suppress Codex's
  own prompt — the one failure mode ctm is built to avoid. OpenCode and Claude Code are
  unaffected.
- **Codex assistant text arrives once per turn** (its hooks expose the final message,
  not a stream); tool activity still appears as it happens.
