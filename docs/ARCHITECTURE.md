...
### AskUserQuestion workflow (ADR-012 UI + ADR-014 E delivery)

1. The `AskUserQuestion` `PreToolUse` hook **blocks** and sends a `question_request` over the socket (carrying the `questions`), recording the originating client in `pending_question_clients` (ADR-014 E1). The hook's own `tool_start` for AskUserQuestion is suppressed so the question renders only once, from this request.
2. The daemon renders each question as a Telegram message with inline buttons and stores a `PendingQuestion` keyed by session ID. A render failure (message id `0`) is surfaced in the topic, not silently swallowed (E4).
3. Button taps set tentative answers via `TentativeAnswer::Option` or `TentativeAnswer::MultiOption` (toggle). Free-text replies set `TentativeAnswer::FreeText`. All mutations use a per-key `Mutex<PendingQuestion>`. The ADR-012 tentative-selection UX (including the "Submit All" review) is retained.
4. When all questions have tentative answers, a summary review screen is sent with "Submit All" and "Change QN" buttons.
5. "Submit All" delivery is decided by `classify_submit`:
   - **All option/multi-select answers** → build an answers map (question text → label, multi-select labels joined with a bare comma) and write a `question_response` back to the blocked hook, which returns `permissionDecision: "allow"` + `updatedInput { questions, answers }`. **No keystrokes, no TUI, no race** (E1/E2) — the structured path; ~6 orders of magnitude faster than the old key-by-key injection.
   - **Any free-text answer** → there is no structured contract for free-text (E3), so the hook is released with a bare `allow` (sentinel `__freetext_fallback__`) and the answers are injected via the **sole remaining** keystroke path after a short TUI-render wait.
   - **No registered hook client** (e.g. timed out) → the user is notified rather than blindly injecting.
   "Change QN" returns to the question.
...
2. For a true termination (`clear`, `logout`, `prompt_input_exit`, `bypass_permissions_disabled`, `other`, unknown):
   ...
   - Mark the session `ended` in SQLite (atomic transaction with approval expiry) and evict its `pending_question_clients` / `pending_approval_clients` entries.
...
| `pending_question_clients` | `Arc<RwLock<HashMap>>` | ADR-014 E1: session_id -> client_id of the hook blocked on an AskUserQuestion |
