...
**hook.rs** -- Hook event processing, the entry point for `ctm hook`. Reads stdin with a 1 MiB size limit, parses the JSON into a `HookEvent`, validates the session ID, detects the current tmux session from `$TMUX`, and builds one or more `BridgeMessage` values. For `PreToolUse` events that require approval, it calls `send_and_wait` to block on the socket for up to 5 minutes. For `PreToolUse` events on the `AskUserQuestion` tool, the hook does **not** block — it falls through to `None` so Claude renders its native question widget in the CLI; the daemon mirrors the question to Telegram from the ordinary `tool_start` (ADR-015, superseding ADR-014 PR-E — the blocking `get_question_hook_output`/`updatedInput` path was removed because `updatedInput` suppresses the CLI render and broke ctm's both-surfaces model). The hook always emits a `ToolResult` for AskUserQuestion on `PostToolUse`, which the daemon uses as the "answered at the terminal" signal. For `Stop` events, it extracts the transcript summary or falls back to reading the JSONL transcript file, with `transcript_path` validated against path traversal before opening (ADR-010 S-1). For `SessionEnd` events, it sends a `session_end` message carrying the `session_exit_reason` (deserialized via a serde alias, ADR-014 A2) and cleans up the transcript state file. Always sends a `session_start` message first (the daemon deduplicates).
...
3. The daemon creates a `PendingApproval` in SQLite, sends it to Telegram with an inline keyboard at **Critical** queue priority (ADR-014 B1, so a backlog can't delay it).
4. `handle_callback_query()` resolves the approval **only if this tap transitioned the pending row** (ADR-014 B2 — double-taps are no-ops), routes the response to the specific socket client (ADR-010 S-2), edits the message to a static **decision+time** line with the keyboard removed (ADR-014 B3), and answers the callback. A callback for an unknown/expired approval ID is acknowledged with an alert rather than crashing (B5).
5. The hook returns `permissionDecision: "allow"`, `"deny"`, or `"ask"` (fallback to CLI on timeout).

### AskUserQuestion workflow (ADR-012 UI + ADR-015 injection delivery; supersedes ADR-014 PR-E)

ctm's defining model is **both surfaces, either answers**: a question must render on *both* the CLI and Telegram and be answerable from *either*, with no manual interface switch. ADR-014 PR-E broke this by returning the answer via `updatedInput` (which Claude treats as "already answered — don't prompt," suppressing the CLI widget). ADR-015 reverts to injection and fixes the *actual* bug (the blind submit sleep) instead.

1. The `AskUserQuestion` `PreToolUse` hook returns fast (does **not** block), so Claude draws its native widget **in the CLI**. The hook's `tool_start` is **un-suppressed**, so the daemon renders the question to Telegram from the ordinary fire-and-forget `tool_start` mirror (`handle_tool_start → handle_ask_user_question`) — the same rail every other tool uses. The question is now live on both surfaces.
2. The daemon stores a `PendingQuestion` keyed by session ID, arbitrated by a `QuestionLifecycle { Active, Submitting, Resolved }` state machine; every mutating/keyboard-arming path gates on `Active` and re-stales if already resolved. A render failure (message id `0`) is surfaced in the topic, not silently swallowed.
3. Button taps set tentative answers (`TentativeAnswer::Option` / `MultiOption` toggle); free-text replies set `TentativeAnswer::FreeText`. All callback handlers bounds-check `q_idx`/`o_idx` and no-op a stale tap. The ADR-012 tentative-selection UX (including the "Submit All" review) is retained.
4. When all questions have valid tentative answers (`collect_and_validate_answers`, the sole all-answered gate), a review screen is sent with "Submit All" and "Change QN" buttons.
5. **"Submit All" injects the answers into the live CLI widget via tmux `send-keys`** (`inject_answers`, fail-closed `InjectOutcome` — `Success` / `FailedClean` retryable / `FailedDirty` → terminalize). A multi-question AskUserQuestion is one tabbed widget; `inject_answers` drives it by reading the live pane (`capture_pane` → `parse_widget`) at every step, anchored on the bottom-most tab row. The old blind sleeps (`from_millis(2000)` "2s is enough", etc.) are replaced by readiness polling (`READY_POLL_INTERVAL_MS` × `WAIT_STATE_MAX_POLLS`), pressing the confirming Enter only once the review screen is verified present.
6. **First-responder-wins.** If the operator answers at the **CLI** instead, the AskUserQuestion `PostToolUse` `ToolResult` tells the daemon, which edits the Telegram message to "answered at terminal" and removes the buttons (`resolve_pending_question`, idempotent via `Arc::ptr_eq`). The **single-user invariant** (one human, one surface per question) means CLI and Telegram answers never collide — so no capture-pane liveness guard is needed. "Change QN" returns to the question.
...
The daemon runs cleanup every 20 minutes (ADR-014 A6 — raised from 5 min; with event-driven `SessionEnd` teardown the sweep is now a safety net for sessions that die without firing a hook, not the primary teardown path):
...
1. **`session_exit_reason == "resume"` → do nothing** (ADR-014 A3). The session is suspended and will return; tearing down would wrongly destroy its topic.
2. For a true termination (`clear`, `logout`, `prompt_input_exit`, `bypass_permissions_disabled`, `other`, unknown):
   - Send a session-ended notification to the forum topic.
   - When `auto_delete_topics` is on: **clear `thread_id` from the cache and DB first, then delete the forum topic immediately** (ADR-014 A4 — replaces the old two-stage close→delete schedule). Clearing before deleting means a crash mid-teardown leaves at worst a harmless orphaned topic, never a stale `thread_id` pointing at a deleted topic. "Topic already gone" is tolerated.
   - Mark the session `ended` in SQLite (atomic transaction with approval expiry) and evict its `pending_approval_clients` entries.
   - Cascade to child sessions: end all active children by `parent_session_id` (ADR-013 GAP-5).
3. A subsequent resume builds a fresh topic via `ensure_session_exists`; the persisted `custom_title` (ADR-014 A5) is recovered for the new topic name even across a daemon restart. Stale-`thread_id` sends self-heal via the `topic_invalidated` channel (null the mapping, recreate). A resumed child whose parent ended is logged and gets its own topic rather than being dropped.
...
| ADR-013 | Session Hierarchy and tmux Reliability | Parent-child session routing via `extract_parent_session_id`, child-to-parent topic reuse, child message prefix (`get_child_prefix`), `agent_type` tracking, orphan cascade on parent end, spawn/completion notifications with Details button + `.md` file attachment, `is_valid_agent_id()` path traversal prevention, three-tier tmux detection (cache/DB/live), startup cache warming (F8), injection failure warnings (D1/D2), tmux status indicator (D3), two-stage topic lifecycle (close then delete) — *superseded by ADR-014 A4 immediate deletion*, configurable thresholds (`topic_delete_delay_minutes=15`, `inactivity_delete_threshold_minutes=720`), inactivity cleanup sweep, temp file cleanup |
| ADR-014 | Lifecycle Hooks, Approval UX & Input Reliability | **PR-A:** register `SessionEnd` hook (7 hook types); `session_exit_reason` serde alias + `resume` special-case; immediate topic deletion with synchronous `thread_id` clear; persist `custom_title`; cleanup interval 5→20 min; delete fabricated `adaptive_retry` dead code. **PR-E:** ~~structured AskUserQuestion answers via a blocking `PreToolUse` + `updatedInput`~~ — ***superseded by ADR-015*** (the `updatedInput` path suppressed the native CLI render and broke the both-surfaces model; injection restored). **PR-B:** Critical-priority approvals, idempotent `resolve_approval` gating, decision+time audit edit with keyboard removed, `pending_approval_clients` eviction/sweep, graceful stale IDs. **PR-D:** recorded `y/N` trust acknowledgment in the setup wizard (new setups only). |
| ADR-015 | Restore AskUserQuestion to the Bidirectional Mirror | **Supersedes ADR-014 PR-E.** The blocking `updatedInput` path suppressed Claude's native CLI question widget (Claude Code #29547) and the `tool_start` Telegram-render signal, silently degrading ctm's both-surfaces invariant to Telegram-only. Reverted to keystroke injection (ctm's defining rail) and fixed the *actual* multi-select bug: replaced the blind submit sleeps with `tmux capture-pane` readiness polling, driving the one tabbed multi-question widget step-by-step (`inject_answers`, `parse_widget`, fail-closed `InjectOutcome`). First-responder-wins via PostToolUse `ToolResult`; single-user invariant removes the concurrent-answer race. Removed `pending_question_clients` / `QuestionRequest` / `QuestionResponse` / free-text sentinel. |
