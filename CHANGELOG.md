...
- **The daemon could be silently wedged by Telegram rate-limiting it, and nothing noticed** (ADR-023). It was not dead: hook events kept arriving at ~80/min for **42 minutes** while not one handler finished — no error, no warning, not a single Telegram call attempted, every thread parked. Root cause, reproduced in a test: every handler runs under one of 50 permits, and a Telegram `429` was being waited out *inside the handler*. One `createForumTopic` under `retry_after: 40` held a permit for two minutes (three retries), and `create_forum_topic_resilient` multiplied that by four — about eight minutes per topic. `send_message` made things worse by draining the whole send queue *in the calling handler*, so one event inherited every rate-limit pause in the backlog. At 80 events/min the permits were gone in under a minute, and inbound Telegram updates starved with them because both directions share the pool. Nothing was logged because nothing had failed — everything was politely asleep.
- **A handler can no longer wait on Telegram.** Every API call now carries a 20-second budget covering its retries; past it the rate limit is handed back to the caller instead of slept on. Topic creation treats a `429` as "not ready yet" and buffers the event through the existing no-silent-loss path. The send queue drains in a task of its own — `enqueue` pushes, wakes it, and returns — so the one place allowed to wait out a `retry_after` is a task that holds no permit and blocks nobody. Four places that held the tmux injector lock across a Telegram reply now release it first, and a test scans the source so that shape cannot come back. **Under a total Telegram outage the daemon now handles every event at full speed**; messages queue and go out when Telegram recovers.
- **The daemon now watches itself and recovers itself.** It counts what arrives, what is dispatched and what finishes, and keeps a table of handlers currently running and for how long. A watchdog on its own OS thread (so it still runs when no async worker can) judges four failures: the runtime stopped running tasks, handlers are running but none is finishing, events are arriving but none is being handled, and messages are queued but none is going out. An idle daemon is healthy however long it has been quiet. On a stall it logs the in-flight table — the line this incident lacked — and **cancels the stuck handlers, which releases their locks and permits and fixes the stall in place**; only if that does not restore progress does it end its own process so the service manager starts a fresh one (verified against real launchd and systemd), with a budget of five per hour so it can never become a crash loop. **When it happens you get a Telegram message saying so.** `CTM_WATCHDOG=0` turns it off.
- **Telegram's failure modes are testable now.** `CTM_TELEGRAM_API_BASE` points the bot at a stand-in server (production default unchanged), so the inputs that actually break the daemon — `429` with `retry_after`, deleted topics, slow replies — are written down as tests instead of discovered on your phone. The incident reproduces in 60 seconds: against 0.2.53 it ends `completed=0, in_flight=180, free_permits=0`; against this release, `completed=180, in_flight=0, free_permits=50`.

## [0.2.53] - 2026-09-22

### Fixed (reported: `csp resume <id>` from a worktree → "Permission overrides are not supported when resuming a remote task"; a plain resume came back with the wrong directory and permissions)
- **`codex resume <id>` and `codex fork <id>` honour your flags again, inside the app-server** (ADR-022). Since ADR-016 every session runs in the app-server ctm keeps alive (that is what lets Telegram answer its questions), and a thread there obeys rules a plain `codex` never had: a remote `resume`/`fork` ignores `--cd`, refuses every permission flag, and does not restore the flags the session was started with — the Stage-1 thread came back `Workspace` rooted in its original directory with every write into the worktree denied. The `codex` shell function now hands the bare TUI, `resume …` and `fork …` to **`ctm codex-launch`**, which translates the flags into thread settings and applies them through the API *before* attaching: `--dangerously-bypass-approvals-and-sandbox`/`--yolo` → Full Access + never ask; `-s/--sandbox` → the built-in profile; `-a/--ask-for-approval` → the policy; `-C/--cd`, else the directory you are in → the thread's directory; `-c default_permissions=<name>` → a named profile from that directory's `.codex/config.toml`. It prints what it applied (`ctm: session 01a0aba7: directory /opt/wt, Full Access, approval never  (was: …)`), attaches, and reports the exit by thread id. A fork is created through the API with the same settings. Proven against a real isolated daemon and TUI from an *unloaded* thread (the state after every ADR-021 restart): `/status` shows the worktree and Full Access.
- **It fails closed.** Anything it cannot honour is refused with one line, never attached under different permissions than you typed: `--add-dir` and `-c permissions*`/`sandbox*`/`approval_policy` (with the config file to put them in), a bad `-a`/`-s` value, a missing directory, a named profile the daemon does not know for that directory, a failed or unverified update. It refuses to change a thread that is running a turn or waiting on an approval, or that ctm's store shows live in another terminal (`ctm codex-exited --thread <id>` clears a stale one; `codex --remote unix://… resume <id>` attaches as-is). `--help`/`--version` anywhere touch nothing. `--last` and session names resolve to one exact id; a `resume` with no session named gets ctm's numbered picker so the flags still apply. Without an app-server it runs codex locally and says so; `CTM_CODEX_REMOTE=0` opts out.
- **Two ADR-021 preflight gaps** the review found: a failed daemon restart was swallowed (now printed and a `doctor` failure), and an unreadable session store counted as "no live session" (now: never restart on unknown liveness).
- The shell function no longer keeps its own idea of whether a daemon exists (it could disagree with the launcher), probes only the words before `--` (`--help` after `--` was a prompt), and fish's `--remote*` no longer matches `--remote-auth-token-env`. The wrapper is now run by the real `zsh`, `bash` and `fish` in the test suite. Open a new shell after updating.
...
- **Signing is a separate job under a protected environment.** The new `sign-release` job runs under the `release-signing` environment (release tags only) after every build and after Apple signing; it refuses a key the consumers do not pin, self-verifies against the `install.sh` pins, and never executes a candidate. Releases remain fully automatic on a tag push.
...
- **The release record's shape is frozen, and 0.2.45 changed it.** Every client before 0.2.45 parses `stable-<target>.json` with `deny_unknown_fields`; 0.2.45 added a `signing` block, so `ctm update` on an installed 0.2.44 refused the record (`unknown field \`signing\``). The block is gone: the record is byte-compatible with 0.2.44 again and `publish` asserts the exact field set. It was redundant anyway — the record's sha256 already fixes the bytes and therefore the CDHash, and the signing identity is pinned in the consumers. The CDHash and Apple's notary trail remain published in `proof-<target>.json` for auditors.
- From this version the parser tolerates *additive* record fields (identity has always come from `kind`/`schema_version`/`package`/`channel`, and a foreign record is still refused by its `kind`), so a future record can grow without stranding every install — once no pre-0.2.46 client remains.
- Signing itself is unchanged from 0.2.45: both consumers were re-proven against the real 0.2.45 release asset — `install.sh` installs it and refuses ad-hoc bytes with a matching sha256; the Rust updater's verification accepts it (`--ignored` test `a_real_release_asset_is_accepted`, driven by `CTM_TEST_SIGNED_BINARY`).
...
- **`ctm update` and `install.sh` verify the signature, not just the sha256.** On macOS a candidate is refused unless it carries a valid Developer ID signature by team `3T2D2YNTVW` as `us.ctm.cli` with the hardened runtime and a secure timestamp, and Apple confirms the notarization ticket online — all before the atomic swap. The team and identifier are pinned in the binary (`apple_trust.rs`) and in `install.sh`, so a re-signed asset is refused even if the record naming it was replaced too. (0.2.45 also put the identity into the release record; see 0.2.46 for why that was reverted.)
